...
function whoIsBigger(a, b) {
  (a < b) ? console.log(b + " is bigger") : console.log(a + " is bigger")
}
...
splitMe("ciao ora sono un array");
...
function deleteOne(stringa, booleano) {
  if (booleano == true) {
    let stringa1 = stringa.substring(1);
    console.log(stringa1)
  } else {
    let stringa2 = stringa.slice(0, -1);
...
function onlyLetters(stringa) {
  let deletedNumbers = stringa.replace(/[0-9]/g, "");
  console.log(deletedNumbers) 
}

onlyLetters("ciao ho 2 gatti e 7 cavalli")
...
function isThisAnEmail(email) {
  ...
  if (at == true && punto == true) {
    console.log("è una email")
  } else {
    console.log("non è una email")
  }
}

isThisAnEmail("ciao@aaaa.it")
...
console.log(dicesRolled.values);
rollTheDices(3);
...
function howManyDays(value) {
  let dataOggi = new Date;
  let dataSottratta = new Date(value);
  let mancante = dataOggi.getTime() - dataSottratta.getTime();
  let mancanteGiorni = Math.floor(((((mancante/1000)/3600)/24)))
  console.log("sono passati " + mancanteGiorni + " giorni dalla data indicata")
}

howManyDays("04/30/2000");
...
let dataOggi = new Date("04/30/2000");
function isTodayMyBirthday() {
  if (dataOggi === "04/30/2000") {
    console.log("true");
  } else {
    console.log("false");
  }
...
function onlyInLastMillennium() {
  for (let i = 0; i < movies.length; i++) {
    ...
    if (parseInt(movies[i].Year) <= anno) {
      lastMillennium = [];
      lastMillennium.push(movies[i].Year);
...
function sumAllTheYears() {
  let anniSommati = 0;
  for (let i = 0; i < movies.length; i++) {
    anniSommati += parseInt(movies[i].Year);
  }
...
function searchByTitle(stringa) {
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.includes(stringa)) {
      titleFound.push(movies[i].Title);
    }
  }
...
*/
 ...
 const add = function() {
   ...
   let listina = document.createElement("li");
   listina.innerText = "ciaooo";
 }
...
*/
  ...
  let listina = document.getElementsByTagName("li");
   for (let i = 0; i< listina.length; i++) {
    listina[i].innerText = ""
   }
 }
